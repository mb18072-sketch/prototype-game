...
export class BootScene extends Phaser.Scene {
    ...
    create() {
        this.input.once("pointerdown",async () => {
            ...
            let dirHandle = await db.get("assetsDir");

        if (dirHandle) {
            const perm = await dirHandle.queryPermission({ mode: "read" });

            if (perm === "granted") {
                await this.loadAssets(dirHandle);
                return;
            }
        ...
        await db.set("assetsDir", dirHandle);
        await this.loadAssets(dirHandle);
        })
    }

    async loadAssets(dirHandle) {
        const assets = new AssetLoader(this);
        await assets.scanDirectory(dirHandle);
        await assets.loadAll();

        for (const [key, img] of assets._image) {
    const config = assets.getjson(key);
    ...
    if (config?.meta?.type === "spritesheet") {
        ...
        for (const [key,url] of assets._audio) {
            this.load.audio(key,url);
        }

        this.load.start();

        this.registry.set("rootHandle",dirHandle);

        this.registry.set("assets",assets);
